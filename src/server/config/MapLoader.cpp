...
MapLoader::MapInfo MapLoader::loadFromYAML(
    ...
    if (!root["map"]) {
        throw std::runtime_error("Falta la sección 'map' en el YAML del circuito.");
    }

    const auto& mapNode = root["map"];
    MapInfo info;
    info.name = mapNode["name"] ? mapNode["name"].as<std::string>() : "Unnamed Map";
    info.city = mapNode["city"] ? mapNode["city"].as<std::string>() : "Unknown";
    if (mapNode["gravity"] && mapNode["gravity"].IsSequence() && mapNode["gravity"].size() == 2) {
        info.gravity = { mapNode["gravity"][0].as<float>(), mapNode["gravity"][1].as<float>() };
    } else {
    ...
    if (mapNode["walls"]) {
        for (const auto& n : mapNode["walls"]) {
            ...
            std::cout << "Wall id=" << id << " pos=(" << x << "," << y << ") size=("
                      << w << "," << h << ")\n";
        }
    }
    ...
    if (mapNode["hints"]) {
        for (const auto& n : mapNode["hints"]) {
            ...
            std::cout << "Hint id=" << h.getId()
                      << " pos=(" << h.getPosition().x << "," << h.getPosition().y
                      << ") angle=" << h.getAngle() << "\n";
        }
    }
    ...
    std::cout << "Mapa cargado correctamente con "
              << walls.size() << " walls, "
