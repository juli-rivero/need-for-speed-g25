...
Socket::Socket(const char* hostname, const char* servname) {
    ...
    while (resolver.has_next()) {
        const struct addrinfo* addr = resolver.next();

        /* Cerramos el socket si nos quedo abierto de la iteración
         * anterior
         * */
        if (new_skt != -1) ::close(new_skt);
         ...
         * */
        if (connect(new_skt, addr->ai_addr, addr->ai_addrlen) == -1) {
            continue;
        }
    ...
    throw LibError(saved_errno, "socket construction failed (connect to {}:{})",
...
Socket::Socket(const char* servname) {
    ...
    while (resolver.has_next()) {
         ...
         **/
        int optval = 1;
        int s = setsockopt(new_skt, SOL_SOCKET, SO_REUSEADDR, &optval,
                           sizeof(optval));
        if (s == -1) {
            continue;
        }
    ...
    throw LibError(saved_errno, "socket construction failed (listen on {})",
                   (servname ? servname : ""));
}
...
ssize_t Socket::recvsome(void* data, unsigned int sz) {
    chk_skt_or_fail();
    ssize_t s = recv(this->skt, data, sz, 0);
    if (s == 0) {
        /*
...
ssize_t Socket::sendsome(const void* data, unsigned int sz) {
    chk_skt_or_fail();
    /*
     ...
     * (ver más abajo).
     * */
    ssize_t s = send(this->skt, data, sz, MSG_NOSIGNAL);
    if (s == -1) {
        /*
...
ssize_t Socket::recvall(void* data, unsigned int sz) {
    unsigned int received = 0;

    while (received < sz) {
        ssize_t s =
            recvsome(static_cast<char*>(data) + received, sz - received);
        ...
        if (s <= 0) {
            ...
            assert(s == 0);
            if (received) {
                throw LibError(EPIPE, "socket received only {} of {} bytes",
                               received, sz);
            }
            return 0;
        } else {
...
ssize_t Socket::sendall(const void* data, unsigned int sz) {
    unsigned int sent = 0;

    while (sent < sz) {
        ssize_t s = sendsome(static_cast<const char*>(data) + sent, sz - sent);

        /* Véase los comentarios de `Socket::recvall` */
        if (s <= 0) {
            assert(s == 0);
            if (sent) {
                throw LibError(EPIPE, "socket sent only {} of {} bytes", sent,
                               sz);
            }
            return 0;
        } else {
