...
void EditorWindow::setupToolbar() {
  ...
  connect(startAction, &QAction::triggered, this, &EditorWindow::onPlaceStart);
...
void EditorWindow::setupMenuBar() {
  ...
  if (editorMode == EditRaceMode) {
    ...
    QAction* loadImageAction = fileMenu->addAction("Load &Background Image");
    connect(loadImageAction, &QAction::triggered, this,
            &EditorWindow::onLoadBackgroundImage);

    QAction* loadProjectAction = fileMenu->addAction("&Open Project (YAML)");
    loadProjectAction->setShortcut(QKeySequence::Open);
    connect(loadProjectAction, &QAction::triggered, this,
            &EditorWindow::onLoadProject);
  }
...
void EditorWindow::onSaveMap() {
  ...
  if (currentMapFile.isEmpty()) {
    ...
    fileName = QFileDialog::getSaveFileName(this, tr("Save Map Configuration"),
                                            "../maps",
...
void EditorWindow::onLoadBackgroundImage() {
  QString fileName =
      QFileDialog::getOpenFileName(this, tr("Load Background Image"), "../maps",
  ...
  if (mapCanvas) {
    ...
    statusBar()->showMessage(tr("Background image loaded: %1").arg(fileName));
  }
}

void EditorWindow::onLoadProject() {
  QString yamlFileName = QFileDialog::getOpenFileName(
      this, tr("Open Map Project"), "../maps", tr("YAML Files (*.yaml *.yml)"));
  ...
  if (!yamlHandler->loadMap(yamlFileName, backgroundPath, checkpoints, hints,
                            buildings)) {
    QMessageBox::warning(this, "Error", "Failed to load map from YAML file.");
    return;
  }
...
void EditorWindow::updateSelectionInfo() {
  ...
  if (!selected.isEmpty()) {
    ...
    if (CheckpointItem* cp = dynamic_cast<CheckpointItem*>(item)) {
      ...
      info += QString(" | Selected: Building with %1 vertices").arg(vertices);
    }
  }
